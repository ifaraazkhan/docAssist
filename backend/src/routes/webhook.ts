...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    // Always fetch menu protocols and try to match
    ...
    const isGreeting = greetings.some((g) => text.toLowerCase().trim() === g)

    console.log('[webhook] isGreeting:', isGreeting, 'menu protocols:', menuProtocols.length)

    if (isGreeting && menuProtocols.length > 0) {
